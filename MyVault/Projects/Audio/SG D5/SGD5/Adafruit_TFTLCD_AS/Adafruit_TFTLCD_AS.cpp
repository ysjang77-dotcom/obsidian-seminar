// Adafruit_TFTLCD_AS - Modified for Adafruit_GFX_AS compatibility
// Based on Adafruit TFTLCD Library by ladyada/adafruit
// MIT license
...
Adafruit_TFTLCD_AS::Adafruit_TFTLCD_AS(uint8_t cs, uint8_t cd, uint8_t wr, uint8_t rd,
                                       uint8_t reset)
    : Adafruit_GFX_AS(TFTWIDTH, TFTHEIGHT) {
...
Adafruit_TFTLCD_AS::Adafruit_TFTLCD_AS(void) : Adafruit_GFX_AS(TFTWIDTH, TFTHEIGHT) {
  init();
}

// Initialization common to both shield & breakout configs
void Adafruit_TFTLCD_AS::init(void) {
...
void Adafruit_TFTLCD_AS::begin(uint16_t id) {
  uint8_t i = 0;
...
void Adafruit_TFTLCD_AS::reset(void) {
...
void Adafruit_TFTLCD_AS::setAddrWindow(int x1, int y1, int x2, int y2) {
  CS_ACTIVE;
  if (driver == ID_932X) {
...
void Adafruit_TFTLCD_AS::setLR(void) {
  CS_ACTIVE;
...
void Adafruit_TFTLCD_AS::flood(uint16_t color, uint32_t len) {
  uint16_t blocks;
...
void Adafruit_TFTLCD_AS::drawFastHLine(int16_t x, int16_t y, int16_t length,
                                    uint16_t color) {
  int16_t x2;
...
void Adafruit_TFTLCD_AS::drawFastVLine(int16_t x, int16_t y, int16_t length,
                                    uint16_t color) {
  int16_t y2;
...
void Adafruit_TFTLCD_AS::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h,
                               uint16_t fillcolor) {
  int16_t x2, y2;
...
void Adafruit_TFTLCD_AS::fillScreen(uint16_t color) {
...
void Adafruit_TFTLCD_AS::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_TFTLCD_AS::pushColors(uint16_t *data, uint8_t len, boolean first) {
  uint16_t color;
  uint8_t hi, lo;
  CS_ACTIVE;
...
void Adafruit_TFTLCD_AS::setRotation(uint8_t x) {
...
uint16_t Adafruit_TFTLCD_AS::readPixel(int16_t x, int16_t y) {
...
uint16_t Adafruit_TFTLCD_AS::readID(void) {
  uint16_t id;
...
uint32_t Adafruit_TFTLCD_AS::readReg(uint8_t r) {
  uint32_t id;
  uint8_t x;
...
uint16_t Adafruit_TFTLCD_AS::color565(uint8_t r, uint8_t g, uint8_t b) {
...
#ifndef write8
void Adafruit_TFTLCD_AS::write8(uint8_t value) { write8inline(value); }
#endif

#ifdef read8isFunctionalized
uint8_t Adafruit_TFTLCD_AS::read8fn(void) {
  uint8_t result;
  read8inline(result);
  return result;
}
...
#ifndef setWriteDir
void Adafruit_TFTLCD_AS::setWriteDir(void) { setWriteDirInline(); }
#endif

#ifndef setReadDir
void Adafruit_TFTLCD_AS::setReadDir(void) { setReadDirInline(); }
#endif

#ifndef writeRegister8
void Adafruit_TFTLCD_AS::writeRegister8(uint8_t a, uint8_t d) {
  writeRegister8inline(a, d);
}
#endif

#ifndef writeRegister16
void Adafruit_TFTLCD_AS::writeRegister16(uint16_t a, uint16_t d) {
  writeRegister16inline(a, d);
}
#endif

#ifndef writeRegisterPair
void Adafruit_TFTLCD_AS::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
  writeRegisterPairInline(aH, aL, d);
}
#endif

void Adafruit_TFTLCD_AS::writeRegister24(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
void Adafruit_TFTLCD_AS::writeRegister32(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
// Adafruit_GFX_AS required: prepare for fast sequential pixel writes
// For 8-bit parallel TFTLCD, these are no-ops since setAddrWindow handles CS
void Adafruit_TFTLCD_AS::writeBegin(void) {
  // No-op for parallel interface (SPI fast path in GFX_AS not used)
}

void Adafruit_TFTLCD_AS::writeEnd(void) {
  // No-op for parallel interface
}
