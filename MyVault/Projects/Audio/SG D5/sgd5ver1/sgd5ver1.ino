...
****************************************************************************************************
  Change log:
  Modified 02/13/26:- Ver 1.1a ILI9341 LCD 터치 좌표 매핑 수정 (TOUCH_TYPE 도입)
                      LCD_A~D #ifdef 제거, TOUCH_TYPE 0~3 선택 방식으로 변경
                      중복 코드 정리 (색상 정의, #include, 주석 처리된 코드)
...
#include <IRremote.h>
#include <Adafruit_TFTLCD.h> // Hardware-specific library (ILI9341)
#include <TouchScreen.h>


// 버전정보
#define VER_INFO "Ver. 1.1a"

#define CONTROLLER_ID 0x9341 //ILI9341 LCD driver

// =====================================================================
// 터치 좌표 매핑 타입 선택 (ILI9341 LCD용)
// 터치가 작동하지 않거나 좌표가 어긋나면 다른 타입으로 변경하세요.
// TOUCH_TYPE 0: X/Y 스왑 + 양축 반전 (구 LCD_A)
// TOUCH_TYPE 1: X/Y 스왑 + X축만 반전 (구 LCD_B)
// TOUCH_TYPE 2: 스왑없음 + X정방향, Y반전 (구 LCD_C) ← 현재 설정 (핀스왑 후 실측 추정)
// TOUCH_TYPE 3: 스왑없음 + 양축 반전 (구 LCD_D)
// =====================================================================
#define TOUCH_TYPE 0
...
void drawCentreString(const char* text, int x, int y, int fontSize, uint16_t color = WHITE) {
  tft.setTextColor(color);
  tft.setTextSize(fontSize);
  // 글자폭 6px * fontSize * 글자수 로 중앙 위치 계산 (GLCD 기본 폰트 기준)
  int16_t textWidth = strlen(text) * 6 * fontSize;
...
#define YP A1  // must be an analog pin, use "An" notation! (물리적 Y 전극)
#define XM A2  // must be an analog pin, use "An" notation! (물리적 X 전극, LCD_CD 공유 - readTouchOnce()에서 INPUT으로 전환하여 해결)
#define YM 29   // 원복: 7은 LCD 화면 블랭크 → 29로 복원
#define XP 28   // can be a digital pin (물리적 X 전극, 74HC245 경유, LCD_CS=LOW 필요)

#define TS_MINX 0     // 실측: 터치 시 X=0~16 → MINX=0으로 조정
#define TS_MINY 0     // 실측: p.x=109~123 (Power 버튼) → MINY=0으로 설정
#define TS_MAXX 1023  // 실측: 반대편 끝 터치 시 최대값
#define TS_MAXY 1023  // 실측: Power 버튼 Y=977→screen y=304 기준 (1023으로 조정)
...
// 터치 좌표 디버깅: 시리얼 모니터에서 터치 좌표 확인 가능
// 터치가 정상 작동하면 주석 처리하세요
#define DEBUG
...
unsigned long dualCheckMillis = 0; // Stores last time dual mode was checked (10s interval)
...
void refreshSampleRate()
{
  if (screenMode == _MAIN_SCREEN && powerStatus == _POWER_ON) {
    ...
    if ((millis() - displayMillis > INTERVAL_SAMPLE * 1000)) {
      ...
      // LCD 명령은 실제 표시할 때만 실행 (매 루프마다 LCD 핀 구동 방지)
      tft.setRotation(3);
      tft.setCursor(10, 50);
      tft.setTextSize(2);
      tft.setTextColor(WHITE);
  ...
  // 듀얼모드 유지 확인: 10초마다 한 번 체크 (I2C 블로킹으로 터치 이벤트 놓침 방지)
  if (screenMode == _MAIN_SCREEN && powerStatus == _POWER_ON) {
    if (millis() - dualCheckMillis > 10000UL) {
      dualCheckMillis = millis();
      if (checkDualMode() == false) {
        Serial.println("Not Dual mode - restoring");
        setDualMode();
      }
    }
  }
...
// 터치 이벤트 발생했을 때 처리
// 버스 해제 후 1회 측정하는 내부 헬퍼
static TSPoint readTouchOnce() {
  // ★ 진단 테스트: LCD_CS LOW 유지 → 74HC245 활성화
  // (이전: HIGH로 올렸으나 74HC245가 차단되어 XP/YM이 터치패널 미도달)
  ...
  digitalWrite(LCD_CS, HIGH);  // 읽기 완료 후 LCD 비활성화
  pinMode(XM, OUTPUT);
  pinMode(YP, OUTPUT);
  DDRA = 0xFF;
  return p;
...
void procTouch()
{
  // 부팅 직후 3초간 가짜 터치 무시 (핀 안정화 시간)
  if (millis() < 3000) return;

  // ★ 핵심: readTouchOnce() 1번째 호출만 실제 터치 값을 반환함.
  // 2번째 호출부터는 ts.getPoint() 내 Z측정이 LCD_WR을 LOW로 구동해
  // LCD 버스 상태를 오염시켜 X≈950(노이즈)으로 고정됨.
  // → 1회만 읽고 바로 판단하는 방식으로 전환.
  TSPoint p = readTouchOnce();

#ifdef DEBUG
  // 터치 가능 범위(p.x < 960)일 때만 출력 → 버튼별 실측용
  if (p.x < 960) {
    static unsigned long rawPrintMillis = 0;
    if (millis() - rawPrintMillis > 200) {
      rawPrintMillis = millis();
      Serial.print("px="); Serial.print(p.x);
      Serial.print(" py="); Serial.println(p.y);
    }
  }
#endif

  // 연속 5회 확인 (손 뗄 때 노이즈 제거: ~100ms 유지해야 유효)
  static int touchCount = 0;
  if (p.x >= 935) { touchCount = 0; return; }  // 임계값 935, 비터치 시 카운트 리셋
  touchCount++;
  ...
  if (true) {  // 감지 통과
...
#ifdef DEBUG
    // RAW 값과 4가지 매핑 결과를 모두 출력
    int rx = p.x, ry = p.y;
    Serial.print("*** TOUCH! RAW("); Serial.print(rx);
    Serial.print(","); Serial.print(ry);
    Serial.print(") Z="); Serial.println(p.z);
    Serial.print("T0: x="); Serial.print(240 - (int)map(ry, TS_MINX, TS_MAXX, 240, 0));
    Serial.print(" y="); Serial.println(320 - (int)map(rx, TS_MINY, TS_MAXY, 320, 0));
    Serial.print("T1: x="); Serial.print((int)map(ry, TS_MINX, TS_MAXX, 240, 0));
    Serial.print(" y="); Serial.println(320 - (int)map(rx, TS_MINY, TS_MAXY, 320, 0));
    Serial.print("T2: x="); Serial.print((int)map(rx, TS_MINX, TS_MAXX, 240, 0));
    Serial.print(" y="); Serial.println(320 - (int)map(ry, TS_MINY, TS_MAXY, 320, 0));
    Serial.print("T3: x="); Serial.print(240 - (int)map(rx, TS_MINX, TS_MAXX, 240, 0));
    Serial.print(" y="); Serial.println(320 - (int)map(ry, TS_MINY, TS_MAXY, 320, 0));
    Serial.println("---");
#endif

    // TOUCH_TYPE에 따른 좌표 매핑
    // tft.width()/height() 대신 하드코딩 값 사용 (setRotation 미호출이므로)
#if TOUCH_TYPE == 0
    // 타입0: X/Y 스왑 + 양축 반전
    int tmp_x = p.x;
    p.x = 240 - (map(p.y, TS_MINX, TS_MAXX, 240, 0));
    p.y = 320 - (map(tmp_x, TS_MINY, TS_MAXY, 320, 0));
#elif TOUCH_TYPE == 1
    // 타입1: X/Y 스왑 + X축만 반전
    int tmp_x = p.x;
    p.x = (map(p.y, TS_MINX, TS_MAXX, 240, 0));
    p.y = 320 - (map(tmp_x, TS_MINY, TS_MAXY, 320, 0));
#elif TOUCH_TYPE == 2
    // 타입2: 스왑없음 + X정방향, Y반전
    // raw X=0 → 화면 x=240 (오른쪽), raw Y=120 → 화면 y=0 (상단)
    p.x = (map(p.x, TS_MINX, TS_MAXX, 240, 0));
    p.y = 320 - (map(p.y, TS_MINY, TS_MAXY, 320, 0));
#elif TOUCH_TYPE == 3
    // 타입3: 스왑없음 + 양축 반전 (ILI9341 기본 추천)
    p.x = 240 - (map(p.x, TS_MINX, TS_MAXX, 240, 0));
    p.y = 320 - (map(p.y, TS_MINY, TS_MAXY, 320, 0));
#endif
#ifdef DEBUG
    // 매핑된 좌표 및 버튼 판별 출력
    ...
    // 버튼 범위 출력 (x=182~240, 버튼별 y 범위)
    Serial.print("    BTN_X_OK="); Serial.println((p.x > 182 && p.x <= 240) ? "YES" : "NO");
    if (p.x > 182 && p.x <= 240) {
      if      (p.y > 10 && p.y < 58)   Serial.println("    => Vol+ 버튼");
      else if (p.y > 73 && p.y < 121)  Serial.println("    => Vol- 버튼");
      else if (p.y > 136 && p.y < 184) Serial.println("    => Input 버튼");
      else if (p.y > 199 && p.y < 247) Serial.println("    => Menu 버튼");
      else if (p.y > 262 && p.y < 310) Serial.println("    => Power 버튼");
      else                              Serial.println("    => 버튼 사이 (미인식)");
    }
#endif
...
void powerOn()
{
  ...
  Serial.println(F("DBG: initLCD start"));

  //LCD 초기화 - 메인화면
  initLCD();
  Serial.println(F("DBG: initLCD done"));
  ...
  //듀얼 모노 셋팅
  setDualMode();
  debounceMillis = millis();  // powerOn 완료 직후 1초간 터치 무시
}
...
void powerOff()
{
  //파워 오프 클릭후 9018 뮤트
  bMuteStatus = true;
  Serial.println(F("DBG: muteDAC start"));
  muteDAC(true);
  Serial.println(F("DBG: muteDAC done"));
  delay(100);

  //LCD 초기화 - Blank
  Serial.println(F("DBG: tft.reset start"));
  tft.reset();
  Serial.println(F("DBG: tft.begin start"));
  tft.begin(CONTROLLER_ID);
  Serial.println(F("DBG: tft.begin done"));
  tft.setRotation(3);
  Serial.println(F("DBG: fillScreen BLACK"));
  tft.fillScreen(BLACK);
  Serial.println(F("DBG: fillScreen done"));
...
/////////////////////////////////////////// 메인프로그램 시작 - Setup  /////////////////////////////////////////////////////
void setup() {
  // ★ 진단: pin7이 74HC245 DIR 또는 LCD 관련 핀일 수 있음 → HIGH로 고정
  pinMode(7, OUTPUT);
  digitalWrite(7, HIGH);
  ...
  Serial.println(F("DBG: irrecv done"));

  //I2C 통신 시작
  //I2C 버스에 마스터로 합류
  Wire.begin();        // Join the I2C bus as a master
  Serial.println(F("DBG: Wire.begin done"));
  //Wire.setClock(400000L);  //I2C 통신속도 설정 : 디폴트값은 100kHz, 고속통신 400kHz

  //전원 Off - 아두이노가 리셋될 때는 9018 전원 오프
  powerStatus = _POWER_OFF;
  Serial.println(F("DBG: calling powerOff"));
  powerOff();
  Serial.println(F("DBG: powerOff done"));
...
void loop() {
  ...
  //LCD 터치 이벤트 처리
  //procTouch();  // ← 진단용 임시 비활성화

  refreshSampleRate();

  if (irrecv.decode(&results)) {
#ifdef DEBUG
    if (results.value != 0xFFFFFFFF) { Serial.print("IR: 0x"); Serial.println(results.value, HEX); }
#endif
