...
# 프롬프트 설계요구
- [목적] 
	- 지정된 로컬 폴더에 있는 다수의 Markdown(`.md`) 파일을 탑재하여 분석에 사용하는 RAG 분석 Python 코드를 작성합니다.
	- 이 코드는 최종적으로 Streamlit과 같은 웹 애플리케이션과 통합되어 사용되므로 VectorDB를 파일로 저장하고 재사용하여 효율성을 극대화해야 합니다.
- [요구사항]
	- 아래 제시된 프롬프팅 핵심기법에 따라서 코드를 작성할 것
	- 생성형AI에게 python 코드생성을 요청하는 프롬프트를 출력할 것
	- RAG 단계를 document load - text split - embedding - vectorDB store - retrieval - prompt & llm - output 으로 나누어서 구성
	- markdown 형식으로 저장된 문서를 파싱하여 VectorDB에 탑재하고, 검색하여 llm을 통해서 시스템 컨텍스트(프롬프트)에 따라서 가공하여 답변을 출력하는 과정을 포함
	- 단순 유사도 검색 외에 문서의 구조 및 메타정보를 포함하여 VectorDB에 탑재되도록 할 것
	- 검색된 문서로 답변을 제공할 때 답변의 출처를 함께 출력하도록 할 것
	- 'docs' 라는 하위폴더에 있는 모든 markdown문서(.md) 파일을 탐색하여 탑재해야 함 
	- 문서는 헤더(#, ##, ###, ####) 표시로 구분되어 있으므로 이것을 기준으로 분할해야 함
	- VectorDB에 탑재된 정보는 파일로 출력(export)하여 재사용이 가능해야 함
	- langchain 을 사용할 것
