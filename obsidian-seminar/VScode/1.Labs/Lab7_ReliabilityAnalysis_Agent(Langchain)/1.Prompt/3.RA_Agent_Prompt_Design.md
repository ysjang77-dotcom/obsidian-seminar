# 요구사항
- [목적] 수명데이터 전처리 및 가공, 최적 수명분포 결정, 수명분포 추정 및 분석을 수행하는 python 코드를 리팩토링하여 Gemini CLI에서 frontend와 backend로 구성된 신뢰성 데이터 분석 AI-agent를 구현하기 위한 시스템 컨텍스트(gemini.md)와 작업 프롬프트 작성
- [활용사례]
	- 관측중단이 존재하는 가속수명 또는 일반수명시험 데이터를 입력받아서 그룹(스트레스)별로 데이터셋을 구분하여 정리하고 테이블로 출력
	- 그룹별로 사용자가 지정하는 분포목록에 대해서 수명분포를 분석하여 적합도 지표와 함께 가장 적합한 분포를 추천하고 추정결과를 시각화
	- 사용자가 지정한 분포에 대해서 분포추정을 수행하여 상세한 분포추정 결과(표, 그림)와 사용자가 요구한 신뢰성 측도(복수개의 B수명, 시간에서의 누적고장확률)를 계산하여 출력
	- 사용자가 지정한 분포에 대해서 분포의 형상모수(또는 척도모수)의 동일성 검정여부를 검토할 수 있는 분석결과를 제시 (형상모수의 추정치와 신뢰구간 테이블 및 시각화 등)
- [AI-agent 구현내용 및 범위]
	- 분석코드를 활용하여 backend를 개발하고, 사용자가 데이터 업로드, 분석 환경설정, 분석실행 및 결과리포트 조회를 할 수 있는 frontend (GUI)를 개발
	- 챗봇 형식으로 사용자가 요구사항을 입력하면 AI-agent가 적절한 도구를 선택하여 분석함수를 호출하고 결과를 실행하여 저장하며, 사용자의 판단에 따라서 분석이 진행되도록 사용자 인터페이스를 구현
- [요구사항]
	...
	- frontend는 streamlit을 사용하고, backend는 fastapi를 사용함.
	- langchain 을 사용하여 tools, Agent, toolkits, agentexecutor 를 구현할 것
	- 도구(tool) 개발시 첨부한 python 분석코드(RA_code_v6.py)를 활용할 것. 코드에 구현된 함수의 내용, 산출물 형식을 변경하지 말고 AI-agent 도구정의 및 인터페이스에 필요한 부분만 수정할 것
	- 임베딩 및 llm 모델은 구글의 gemini (gemini-2.5-flash 또는 gemini-2.5-pro) 모델을 사용할 것
	- AI-Agent는 대화형식(chat)으로 진행되며, 분석 및 대화의 히스토리를 모두 저장하여 과거 맥락을 고려하여 대화가 이루어짐
	- 한글로 작성할 것
	- 그 외에 agent 개발을 수행하는데 필요한 추가 요구사항, 논리적으로 보완할 점이나 고려할 사항을 추가해서 프롬프트를 설계할 것
