# -*- coding: utf-8 -*-
"""
수명 데이터 분석 자동화 스크립트 (Life Data Analysis Automation Script)
...
def preprocess_and_summarize_data(file_path: str) -> Dict[str, Dict[str, list]]:
    """
    CSV 파일에서 수명 데이터를 로드하고, 스트레스 그룹별로 전처리 및 요약합니다.
    ...
    if not required_cols.issubset(df.columns):
        ...
        logging.error(f"필수 컬럼이 누락되었습니다: {missing}")
...
def find_best_distribution(grouped_data: Dict[str, Dict[str, list]],
    ...
    for group_name, data in grouped_data.items():
        ...
        fitter_obj = Fit_Everything(
            ...
            print_results=False,  # 결과는 직접 포맷하여 출력
        ...
        try:
            ...
            plt.close('all')  # 메모리 누수 방지를 위해 모든 그림 닫기
...
def analyze_single_distribution(grouped_data: Dict[str, Dict[str, list]],
    ...
    if failure_prob_times:
        ...
        for t in failure_prob_times:
            # 신뢰성(SF)에 대한 신뢰구간을 계산한 후 CDF로 변환
...
if __name__ == "__main__":
    ...
    # --- 분석 시나리오 설정 ---
    # 0. 테스트용 더미 데이터 생성 (실제 분석 시에는 이 부분을 주석 처리하고 파일 경로를 지정)
    # dummy_data = {
    #    'time': [110, 135, 140, 148, 155, 190, 210, 120, 141, 149, 158, 195, 220, 230, 240, 255, 290, 310, 80, 91, 95, 101, 115, 122, 130, 139, 142, 155],
    #    'status': ['failure']*7 + ['censored']*2 + ['failure']*8 + ['censored']*2 + ['failure']*9,
    #    'stress': [100]*9 + [120]*10 + [150]*9
    #}
    #dummy_df = pd.DataFrame(dummy_data)
    #DUMMY_FILE_PATH = "alt_data_example.csv"
    ...
    try:
        # 1단계 실행
        grouped_life_data = preprocess_and_summarize_data(file_path=FILE_PATH)
        ...
        # 3단계 실행 (사용자가 최적 분포 결과를 보고 선택하는 것을 시뮬레이션)
        # 예시: 모든 그룹에 대해 최적 분포로 추천된 분포로 상세 분석 수행
        ...
        if is_alt_data:
            ...
            # 모든 그룹에서 가장 빈번하게 최적 분포로 선정된 분포를 찾음
            ...
            if best_dists:
                ...
                selected_dist = "Weibull_2P" # 기본값
                print(f"최적 분포를 결정할 수 없어 기본값인 '{selected_dist}'를 모든 그룹에 적용합니다.")
        else: # 일반 수명 데이터
            selected_dist = best_fit_results['group_1']['best_dist']
