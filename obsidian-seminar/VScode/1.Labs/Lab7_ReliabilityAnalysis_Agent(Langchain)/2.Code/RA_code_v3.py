# -*- coding: utf-8 -*-
"""
수명 데이터 분석 자동화 스크립트 (Life Data Analysis Automation Script) - v3
...
def preprocess_and_summarize_data(
    ...
    # *** 개선된 부분: 사용자 지정 지시자를 표준 값으로 변환 ***
    failure_indicator = status_indicators['failure']
    censor_indicator = status_indicators['censored']
    
    # status 컬럼의 데이터 타입을 문자열로 통일하여 비교 오류 방지
    df['status'] = df['status'].astype(str)
    failure_indicator = str(failure_indicator)
    censor_indicator = str(censor_indicator)

    status_map = {
        failure_indicator: 'failure',
        censor_indicator: 'censored'
    }
    df['status'] = df['status'].map(status_map)

    # 변환 후 유효하지 않은 값이 있는지 확인
...
if __name__ == "__main__":
    ...
    # DUMMY_FILE_PATH = "alt_data_example_v3.xlsx"
