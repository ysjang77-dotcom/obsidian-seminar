# -*- coding: utf-8 -*-
"""
수명 데이터 분석 자동화 스크립트 (Life Data Analysis Automation Script) - v4
...
def find_best_distribution(grouped_data: Dict[str, Dict[str, list]],
    ...
    # Fit_Everything은 모든 분포를 피팅하므로, exclude 목록은 사용하지 않고 나중에 결과를 필터링합니다.
    analysis_results = {}
    ...
    for group_name, data in grouped_data.items():
        ...
        # *** 개선된 부분: 그림 생성을 위해 show_* 플래그를 True로 설정 ***
        ...
        # *** 개선된 부분: 사용자가 지정한 분포 목록으로 결과 필터링 ***
        ...
        # *** 개선된 부분: 그림 객체 존재 여부 확인 후 저장 ***
        try:
            if hasattr(fitter_obj_all, 'probability_plot'):
                fitter_obj_all.probability_plot.savefig(os.path.join(RESULTS_DIR, f"{group_name}_all_probability_plots.png"))
            if hasattr(fitter_obj_all, 'histogram_plot'):
                fitter_obj_all.histogram_plot.savefig(os.path.join(RESULTS_DIR, f"{group_name}_histogram_plot.png"))
            if hasattr(fitter_obj_all, 'PP_plot'):
                fitter_obj_all.PP_plot.savefig(os.path.join(RESULTS_DIR, f"{group_name}_pp_plots.png"))
            if hasattr(fitter_obj_all, 'best_distribution_probability_plot'):
                fitter_obj_all.best_distribution_probability_plot.savefig(os.path.join(RESULTS_DIR, f"{group_name}_best_dist_prob_plot.png"))
            plt.close('all')
            logging.info(f"그룹 '{group_name}'의 분석 그림들을 '{RESULTS_DIR}' 폴더에 저장했습니다.")
        except Exception as e:
            logging.error(f"그룹 '{group_name}'의 그림 저장 중 오류 발생: {e}")

        # *** 개선된 부분: 최적 분포로 다시 피팅하여 상세 객체 저장 ***
        ...
        if fitter_class:
            final_fitter_obj = fitter_class(
                ...
                right_censored=data['right_censored'],
                print_results=False,
                show_probability_plot=True # 그림 객체 생성을 위해 True
            )
            plt.close(final_fitter_obj.probability_plot.figure) # 그림은 저장하지 않고 닫음
        else:
        ...
        analysis_results[group_name] = {
            ...
            'fitter_object': final_fitter_obj # 상세 피팅 객체 저장
        }
...
def analyze_single_distribution(grouped_data: Dict[str, Dict[str, list]],
    ...
    # *** 개선된 부분: 그림 생성을 위해 show_probability_plot=True로 설정 ***
    ...
    # *** 개선된 부분: 그림 객체 존재 여부 확인 후 저장 및 닫기 ***
    if hasattr(fitter_obj, 'probability_plot'):
...
if __name__ == "__main__":
    ...
    # DUMMY_FILE_PATH = "alt_data_example_v4.xlsx"
