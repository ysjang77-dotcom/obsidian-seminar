...
def find_best_distribution_func(failures: list, right_censored: list) -> dict:
    ...
    bic_results = {name: fitter(failures=failures, right_censored=right_censored).BIC 
                   if failures else np.inf for name, fitter in distributions.items()}
    bic_df = pd.DataFrame(list(bic_results.items()), columns=["Distribution", "BIC"]).sort_values(by="BIC")
    best_dist_name = bic_df.iloc[0]["Distribution"]
    return {"best_distribution_name": best_dist_name, "bic_results": bic_df}
...
def analyze_distribution_func(failures: list, right_censored: list, dist_name: str, p_values: list, t_values: list, cl: float) -> Dict[str, Any]:
    ...
    if dist_name not in fitter_map:
        raise ValueError(f"지원하지 않는 분포입니다: {dist_name}")

    fitter = fitter_map[dist_name](failures=failures, right_censored=right_censored)
    param_df = pd.DataFrame({
        'Parameter': fitter.parameters.index, 'Point Estimate': fitter.parameters.values,
        'Lower CI': fitter.lower_CI.values, 'Upper CI': fitter.upper_CI.values
    })
    fig, ax = plt.subplots()
    fitter.distribution.plot_probability(ax=ax)
    buf = io.BytesIO()
    fig.savefig(buf, format="png")
    buf.seek(0)
    plot_image = buf.getvalue()
    plt.close(fig)
    b_life_data = [[p, *fitter.distribution.CDF(CI_type='time', CI_y=p/100, CI=cl)] for p in p_values]
    b_life_df = pd.DataFrame(b_life_data, columns=['p', 'Lower', 'Point', 'Upper'])
    cdf_data = [[t, *fitter.distribution.CDF(CI_type='reliability', CI_x=t, CI=cl)] for t in t_values]
...
def analyze_best_distribution_node(state: GraphState):
    # GraphState의 필드 이름이 analyze_distribution_func의 인자 이름과 일치하므로 **state 전달 가능
    state['analysis_results'] = analyze_distribution_func(**state) 
    return state
...
**2. 최적 분포 결정**\n가장 낮은 BIC 값을 보인 **{state['best_dist_name']}** 분포가 최적 분포로 선정되었습니다.\n{state['bic_results'].to_markdown(index=False)}
**3. 최적 분포 분석 결과 ({state['best_dist_name']})**
*   **추정 파라미터 ({state['cl']*100:.0f}% CI):**\n{state['analysis_results']['parameter_table'].to_markdown(index=False)}
*   **확률지:**\n*(이미지는 UI에서 별도로 표시됩니다)*
**4. 신뢰성 측도 ({state['cl']*100:.0f}% CI)**
*   **B-Life:**\n{state['analysis_results']['b_life_table'].to_markdown(index=False)}
*   **누적고장확률:**\n{state['analysis_results']['cdf_table'].to_markdown(index=False)}
**5. 결론 요약**\n본 분석 결과, 제공된 데이터는 {state['best_dist_name']} 분포를 따르는 것으로 나타났습니다. B{state['p_values'][0]} 수명은 약 {state['analysis_results']['b_life_table']['Point'].iloc[0]:.2f} 시간으로 추정되며, {state['t_values'][-1]} 시간까지의 누적 고장 확률은 약 {state['analysis_results']['cdf_table']['Point'].iloc[-1]*100:.1f}%로 예측됩니다."""
...
system_message = """You are a helpful assistant for reliability analysis.
The user has already uploaded their lifetime data. This data is automatically available to your tools.
You MUST NOT ask for the data.
Just call the appropriate tool based on the user's request.
For example, if the user asks to 'summarize the data', call the `summarize_data` tool directly without asking for the data source.
"""
